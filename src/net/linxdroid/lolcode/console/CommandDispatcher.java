...
import net.linxdroid.lolcode.console.builtincmd.CmdSettings.Setting;
import net.linxdroid.lolcode.console.builtincmd.FileSys;
import net.linxdroid.lolcode.console.builtincmd.CmdSettings;
import net.linxdroid.lolcode.console.builtincmd.HelpCmd;
import net.linxdroid.lolcode.console.builtincmd.SystemCmd;
...
public class CommandDispatcher implements Runnable, IStdOut
{
	 ...
	 private void doCommandLoop(CmdSettings cmdSettings, FileSys fileSys)
	 {
		...
		while (isRun) {
			...
			else if (builtInCmd == CmdInfo.BuiltInCmd.lolcode) {
				try{
					String apkPath = FileSys.getResolvedPath(fileSys.getResultantPath("/sdcard/lolcode/.libs/LOLInterpreter.apk"));
					apkRunner = new ApkRunner(this, apkPath, args);
					apkRunner.execute();
					apkRunner = null;
				}catch(Exception e){
					printError("E/CommandDispatcher(Main.main): " + e.toString());
				}
			}
